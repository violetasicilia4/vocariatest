 ...
 *
 * ⚠️ MERCADO PAGO — INTEGRACIÓN PENDIENTE (NO PRODUCTIVA). Mantener
 * VITE_PAYMENTS_ENABLED='false' hasta endurecer las funciones de api/ (validación
 * de monto/plan/external_reference, webhook firmado, persistencia segura). La
 * entrega del informe NO debe depender de datos del cliente sin verificación
 * server-side. Ver TODO(mp) en api/create-preference.ts, verify-payment.ts y
 * mp-webhook.ts.
 */
import type { ScoringResult } from '../test/engine/scorer';
import type { PlanId } from '../test/data/profile';
import { readWithTTL, writeWithTTL, remove } from './storage';
...
export function savePendingOrder(order: PendingOrder): void {
  writeWithTTL(PENDING_KEY, order);
}

export function loadPendingOrder(): PendingOrder | null {
  // readWithTTL ya descarta (y limpia) datos vencidos, corruptos o de formato viejo.
  return readWithTTL<PendingOrder>(PENDING_KEY, PENDING_MAX_AGE_MS);
}
